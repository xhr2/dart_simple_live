...
class DouyinSite implements LiveSite {
  ...
  Future<String> generateTTWID() async {
    try {
      ...
      var requestHeaders = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36 NetType/WIFI MicroMessenger/7.0.20.1781(0x6700143B) WindowsWechat(0x63090551) XWEB/6939 Flue',
      };
      ...
      // 检查响应头中是否包含Set-Cookie
      var cookies = response.headers['set-cookie'];
      if (cookies != null && cookies.isNotEmpty) {
        var cookieString = cookies.join(';');
        var cookiesList = cookieString.split(';');
        
        for (var cookie in cookiesList) {
          if (cookie.contains('ttwid=')) {
            var ttwid = cookie.split('=')[1];
            return 'ttwid=' + ttwid;
          }
        }
      }
  ...
  Future<Map<String, dynamic>> getRequestHeaders() async {
    try {
      // 检查是否已有cookie，如果没有则生成TTWID
      if (!headers.containsKey("cookie")) {
        var ttwid = await generateTTWID();
        if (ttwid != null) {
